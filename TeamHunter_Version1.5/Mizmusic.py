...
class Music_Player:
    ...
    def play_my_music(self, seek_slider, equalizer_bar, current_song_label):
        ...
        if self.playlist:
            ...
            # When the song ends, advance to the next song if the playlist is not empty
            if self.playlist:
                pygame.mixer.music.queue(os.path.join(self.sounds_dir, self.playlist[0]))
        else:
            # Optionally handle the case where the playlist is empty
...
class MusicPlayer(QWidget):
    ...
    def next_song(self):
        ...
        if self.speaker.playlist:
            pygame.mixer.music.queue(os.path.join(self.speaker.sounds_dir, self.speaker.playlist[0]))
        else:
