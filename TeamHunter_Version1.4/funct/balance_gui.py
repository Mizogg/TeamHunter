...
from funct import team_balance
...
class BalanceDialog(QDialog):
    ...
    def get_balance(self):
        ...
        if not self.is_valid_btc_address(address):
            self.balance_label.setText("Invalid Bitcoin address. Please enter a valid address.")
            return
        ...
        try:
            ...
            self.balance_label.setText(BTCOUT)
        except Exception as e:
            self.balance_label.setText(f"Error retrieving balance: {str(e)}")
    
    def is_valid_btc_address(self, address):
        # Basic validation for Bitcoin addresses (1, 3, or bc1)
