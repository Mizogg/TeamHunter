...
def hex_to_poetry(hex_string):
    mn_list = []
    n = len(mn_words)
    for i in range(0, len(hex_string), 8):
        x = int(hex_string[i:i + 8], 16)
        w1 = x % n
        w2 = (x // n + w1) % n
        w3 = (x // n // n + w2) % n
        mn_list.extend([mn_words[w1], mn_words[w2], mn_words[w3]])
    return ' '.join(mn_list)
...
def mn_decode(input_str):
    ...
    for i in range(0, len(wlist), 3):
        w1 = mn_words.index(wlist[i])
        w2 = mn_words.index(wlist[i+1]) % n
        w3 = mn_words.index(wlist[i+2]) % n
        x = w1 + n * mn_mod((w2 - w1), n) + n * n * mn_mod((w3 - w2), n)
        out += format(x, '08x')
